...
";
        ...
        public void ShowAllContacts()
        {
            ...
            // FIX: Use List.Sort() with ContactComparer instead of LINQ OrderBy().
            // List.Sort() uses introspective sort (insertion / heapsort / quicksort)
            // which is O(n log n). ContactComparer handles all tie-breaking rules.
            switch (input)
            {
                case "1":
                    entries.Sort(new ContactComparer(ContactComparer.SortField.FirstName));
                    break;
                case "2":
                    entries.Sort(new ContactComparer(ContactComparer.SortField.LastName));
                    break;
                case "3":
                    entries.Sort(new ContactComparer(ContactComparer.SortField.Phone));
                    break;
                case "4":
                    entries.Sort(new ContactComparer(ContactComparer.SortField.Email));
                    break;
                case "M":
                    return;
                default:
        ...
        public void EditContact()
        {
            ...
            // FIX: Replace LINQ .Where() with List.FindAll().
            // Both are O(n) linear searches, but the professor specifically
            // requires the use of List.FindAll() per the .NET documentation note.
            List<Entry> coincidencias = entries.FindAll(e =>
        ...
        public void DeleteContact()
        {
            ...
            // FIX: Replace LINQ .Where() with List.FindAll() (same reason as EditContact).
            List<Entry> coincidencias = entries.FindAll(e =>
                (campo == "nombre" && e.FirstName?.ToLower().Contains(valor) == true) ||
                (campo == "apellido" && e.LastName?.ToLower().Contains(valor) == true) ||
                (campo == "telefono" && e.PhoneNumber?.ToLower().Contains(valor) == true) ||
        ...
        public List<List<Entry>> FindDuplicatedContacts()
        {
            ...
            // FIX: DSU is now a proper separate class (DSU.cs) instead of
            // local functions buried inside this method.
            DSU dsu = new DSU(n);
